...
/**
 * Страница билета, отображающая информацию о заказанном билете и QR-код.
 *
 * @returns {React.FC} Компонент страницы билета.
 */
const TicketPage: React.FC = () => {
  ...
  // Если данные билета отсутствуют, отображаем Placeholder
  if (!sessionId || !movie || !hall || !time || !date || !seats || totalCost === undefined) {
    return (
      <main className="page-container">
        <Placeholder text="Данные о билете отсутствуют." />
      </main>
    );
  }
    ...
    <main className="page-container">
      <section className="ticket">
        ...
        <div className="ticket__info-wrapper">
          ...
          <p className="ticket__info">
            Места: <span className="ticket__details ticket__chairs">{seatString}</span>
          </p>
          <p className="ticket__info">
            В зале: <span className="ticket__details ticket__hall">{hall}</span>
          </p>
          <p className="ticket__info">
            Дата: <span className="ticket__details ticket__date">{formatDate(date)}</span>
          </p>
          <p className="ticket__info">
            Начало сеанса: <span className="ticket__details ticket__start">{formatTime(time)}</span>
          </p>
          <p className="ticket__info">
            Стоимость: <span className="ticket__details ticket__cost">{totalCost.toFixed(2)}</span> рублей
          </p>

          <QRCodeSVG value={JSON.stringify(qrData)} className="ticket__info-qr" />
